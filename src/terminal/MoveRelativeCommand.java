...
public class MoveRelativeCommand extends Command{
	...
	public boolean run(String[] args) {
		...
		try {
			if(action == null) {
				action = new MoveAction<Player>(Player.player, Player.player.REFACTORSTUFFgetAbsoluteX() + Integer.parseInt(args[1]), Player.player.REFACTORSTUFFgetAbsoluteY() + Integer.parseInt(args[2]), true);
...
//		return player.pathToPreview(player.getAbsoluteX() + Integer.parseInt(args[1]), player.getAbsoluteY() + Integer.parseInt(args[2])).pointsToHighlight();
		...
		temp = Player.player.pathToPreview(Player.player.REFACTORSTUFFgetAbsoluteX() + Integer.parseInt(args[1]), Player.player.REFACTORSTUFFgetAbsoluteY() + Integer.parseInt(args[2]));
		if(temp == null) {
